...
import {
  ...
  remoteSetTeamFlags,
  remoteToggleEvent,
  remoteUpdateBigTeamColor,
  remoteUpdateHuntEvent,
...
function migrateLoadedState(state: GameState): GameState {
  ...
  return {
    ...state,
    bigTeams: state.bigTeams?.length ? state.bigTeams : seed.bigTeams,
    events: (state.events ?? []).map((e, i) => ({
      ...e,
      place: e.place ?? seed.events[i]?.place ?? '',
    })),
  };
}
...
class GameStore {
  ...
  async init() {
    ...
    if (this.usingRemote) {
      ...
      subscribeRemoteGame(() => this.scheduleRemoteRefresh());
      setInterval(() => {
      ...
      try {
        ...
        if (raw) this.state = migrateLoadedState(JSON.parse(raw) as GameState);
      } catch {
  ...
  async addCustomEvent(timeLabel: string, title: string, body: string, place = '') {
    if (this.usingRemote) {
      const r = await remoteAddCustomEvent(this.session, timeLabel, title, body, place);
      if (!r.ok) throw new Error(r.error);
      await this.refreshFromRemote();
      return;
    }
    const events = [
      ...this.state.events,
      { id: `${Date.now()}`, timeLabel, title, body, place, active: false },
    ];
    await this.persistLocal({
      ...this.state,
      events,
  ...
  async updateHuntEvent(
    eventId: string,
    patch: { timeLabel: string; title: string; body: string; place: string },
  ) {
    if (this.usingRemote) {
      const r = await remoteUpdateHuntEvent(this.session, eventId, patch);
      if (!r.ok) throw new Error(r.error);
      await this.refreshFromRemote();
      return;
    }
  ...
  async updateBigTeamColor(code: BigTeamCode, color: string) {
    if (this.usingRemote) {
      const r = await remoteUpdateBigTeamColor(this.session, code, color);
      if (!r.ok) throw new Error(r.error);
      await this.refreshFromRemote();
      return;
    }
    const bigTeams = this.state.bigTeams.map((b) =>
      b.code === code ? { ...b, color } : b,
    );
    await this.persistLocal({
      ...this.state,
      bigTeams,
