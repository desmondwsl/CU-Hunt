...
export async function fetchRemoteGameState(): Promise<GameState> {
  ...
  const bigTeams: BigTeamInfo[] = BIG_TEAMS.map((bt) => {
    const row = (bigTeamsRes.data ?? []).find((r) => (r as Record<string, unknown>).code === bt.code) as
      | Record<string, unknown>
      | undefined;
    return {
      code: bt.code,
      fullName: String(row?.full_name ?? bt.fullName),
      color: String(row?.color ?? TeamColors[bt.code]),
    };
  });
...
export function subscribeRemoteGame(onChange: () => void): () => void {
  ...
  const channel = client
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'big_teams' }, onChange)
    .subscribe();
  return () => {
...
export async function remoteUpdateHuntEvent(
  session: Session | null,
  eventId: string,
  ...
  const client = requireClient();
  const { role, pin } = credsOrThrow(session);
  const { data, error } = await client.rpc('update_hunt_event', {
    p_role: role,
    p_pin: pin,
    p_event_id: eventId,
    p_time_label: patch.timeLabel,
    p_title: patch.title,
    p_body: patch.body,
    p_place: patch.place,
  });
...
export async function remoteUpdateBigTeamColor(
  session: Session | null,
  code: BigTeamCode,
  color: string,
) {
  const client = requireClient();
  const { role, pin } = credsOrThrow(session);
  const { data, error } = await client.rpc('update_big_team_color', {
    p_role: role,
    p_pin: pin,
    p_code: code,
    p_color: color,
